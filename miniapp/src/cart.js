...
export function changeQty(state, product, delta) { const id = product.id; const next = Math.max(0, cartQty(state, id) + delta); if (!next) delete state.cart[id]; else state.cart[id] = { id, produto_id: product.produto_id || id, name: product.name, nome: product.name, price: product.price, preco: product.price, image: product.image, unit: product.unit, section: product.section, points: product.points, quantity: next }; saveCart(state); return next; }
export function clearCart(state) { state.cart = {}; saveCart(state); }
export function cartPayload(state) { return cartItems(state).map(item => ({ produto_id: item.id, id: item.id, nome: item.name, quantidade: item.quantity, preco: item.price })); }
